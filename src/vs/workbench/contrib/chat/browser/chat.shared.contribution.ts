...
configurationRegistry.registerConfiguration({
	...
	properties: {
		...
		[ClaudePreferAgentHostAgentsSettingId]: {
			type: 'boolean',
			markdownDescription: nls.localize('chat.agents.claude.preferAgentHost', "When enabled, Claude sessions opened from the Agents Window run inside the agent host process instead of the GitHub Copilot Chat extension. Only one Claude implementation surfaces per window. Requires `#chat.agentHost.enabled#`. BatikCode defaults this to on so the Claude Code agent surfaces whenever its SDK is available."),
			default: true,
			tags: ['experimental'],
			experiment: { mode: 'startup' },
		},
